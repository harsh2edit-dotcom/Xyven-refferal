...
async function api(path, options = {}) {
  ...
  // If server sends a new CSRF token in response, save it
  if (data && data.csrf) sessionStorage.setItem('xyven_csrf', data.csrf);
...
function initAuthPage() {
  ...
  // Bootstrap CSRF token on page load (public endpoint)
  api('/api/me').then((d) => {
...
function initAppPage() {
  ...
  const logout = async () => {
    ...
    sessionStorage.removeItem('xyven_csrf');
    location.href = '/login';
  };
...
function initAdminPage() {
  ...
  // Auto check admin session on load
