...
const I18N = {
  ...
  hi: {
    ...
    aboutText: 'XYVEN Referral एक रेफ़रल-आधारित रिवॉर्ड्स प्लेटफ़ॉर्म है जो असली नए सदस्यों को आमंत्रित करके कमाने में मदद करता है। इनाम, निकासी और खाता गतिविधि नियमों और सत्यापन के अधीन हैं।',
  ...
  hg: {
    ...
    referralHistory: 'Referral History',
    deposit: 'Deposit', depositRequirement: 'Deposit Requirement',
    depositExplainer: 'Withdrawal se pehle ₹50 ka verified deposit zaroori hai. Ye platform ko fake accounts se bachata hai. Sirf verified payments count hote hain.',
    submitDeposit: 'Deposit Submit Karo', method: 'Method', refId: 'Payment Reference ID',
    note: 'Note (optional)', submit: 'Submit Karo',
    depositNote: 'Deposit admin dwara reference ke against verify hoti hai. Verification ke baad hi requirement complete hoti hai.',
    depositHistory: 'Deposit History',
    profile: 'Profile', referralCode: 'Referral Code', joined: 'Account Created',
    support: 'Help Chahiye?', about: 'About',
    aboutText: 'XYVEN Referral ek referral-based rewards platform hai jo genuine naye members ko invite karke earn karne me help karta hai. Rewards, withdrawals aur account activity rules aur verification ke subject hain.',
    logout: 'Logout', notifications: 'Notifications', markAllRead: 'Sab read mark karo',
    home: 'Home',
    accountCreated: 'Account ban gaya',
    welcome: 'Wapas aane par swagat',
  },
};
...
async function api(path, options = {}) {
  ...
  const csrf = sessionStorage.getItem('xyven_csrf');
...
/* ==========================================================================
   Background canvas (lightweight particles)
   ========================================================================== */
...
function modal(title, bodyHTML, onConfirm, confirmText = 'Confirm', danger = false) {
  ...
  document.getElementById('modal-bg').onclick = (e) => { if (e.target.id === 'modal-bg') close(); };
  document.getElementById('modal-confirm').onclick = async () => {
    try { await onConfirm(); close(); } catch (_) {}
  };
}
...
function initAuthPage() {
    ...
    try {
      const res = await api('/api/signup', {
        ...
        body: JSON.stringify({
          ...
          referralCode: (fd.get('referralCode') || '').trim().toUpperCase(),
  ...
  // Bootstrap CSRF by hitting /api/me
  api('/api/me').then((d) => d.csrf && sessionStorage.setItem('xyven_csrf', d.csrf)).catch(() => {});
}
...
function initAppPage() {
  ...
  let state = {
    user: null, summary: null, referrals: null, deposits: null, withdrawals: null,
    notifications: [], unread: 0, currentView: 'home',
  };
  ...
  document.getElementById('bell').addEventListener('click', () => showView('notifications'));
  ...
  async function loadSummary() {
    ...
    for (const [k, [val, money]] of Object.entries(cards)) {
      ...
      animateCounter(el, val, money);
    }
    document.getElementById('balance-pill').textContent = fmtMoney(d.balance);

    const sub = document.getElementById('wd-submit');
    sub.disabled = !d.checks.canWithdraw;
  ...
  async function loadProfile() {
    ...
    state.user = d.user;
    document.getElementById('avatar').textContent = (d.user.name || '?').charAt(0).toUpperCase();
    document.getElementById('pf-name').textContent = d.user.name;
    document.getElementById('pf-username').textContent = '@' + d.user.username;
    document.getElementById('pf-email').textContent = d.user.email;
    document.getElementById('pf-code').textContent = d.user.referralCode;
    document.getElementById('pf-joined').textContent = fmtDate(d.user.createdAt);
    document.getElementById('welcome').textContent = t('welcome') + ', ' + d.user.name;
  }
  ...
  async function loadRefer() {
    ...
    state.referrals = d;
    document.getElementById('ref-code').textContent = d.referralCode;
    document.getElementById('ref-link').value = d.referralLink;
    ...
    if (!d.history.length) { wrap.innerHTML = emptyState('No referrals yet. Share your code to get started.', 'users'); }
    else {
  ...
  document.getElementById('share-btn')?.addEventListener('click', async () => {
    ...
    if (!data) return;
    if (navigator.share) {
      try { await navigator.share({ title: 'XYVEN Referral', text: data.shareText }); }
      catch (_) {}
    } else {
  ...
  async function loadDeposits() {
    ...
    state.deposits = d;
    document.getElementById('dep-required').textContent = '₹' + (d.required / 100);
      ...
      wrap.innerHTML = `<table><thead><tr><th>Request ID</th><th>Amount</th><th>UPI ID</th><th>Status</th><th>Date</th></tr></thead><tbody>` +
        d.history.map((r) => `
          <tr>
            <td class="mono-cell">${esc(r.request_id)}</td>
            <td>${fmtMoney(r.amount_paise)}</td>
            <td class="mono-cell">${esc(r.upi_id)}</td>
            <td><span class="${statusCls(r.status)}">${esc(r.status)}</span></td>
            <td class="mono-cell">${fmtDate(r.created_at)}</td>
          </tr>`).join('') + `</tbody></table>`;
    }
  ...
  document.getElementById('coupon-form')?.addEventListener('submit', async (e) => {
    ...
    const fd = new FormData(e.target);
    try {
      const r = await api('/api/coupons/redeem', {
        method: 'POST',
  ...
  function notifIcon(type) {
    return ({
      reward: 'gift', referral: 'users', withdrawal: 'banknote', deposit: 'arrow-down-circle',
      security: 'shield-alert', account: 'user-check', announcement: 'megaphone',
    })[type] || 'bell';
  }
  ...
  async function init() {
    try {
      ...
      sessionStorage.setItem('xyven_csrf', me.csrf);
      state.user = me.user;
      document.getElementById('balance-pill').textContent = fmtMoney(me.user.balance);
      document.getElementById('welcome').textContent = t('welcome') + ', ' + me.user.name;
      await loadSummary();
...
function initAdminPage() {
  ...
  document.getElementById('admin-logout').addEventListener('click', async () => {
   t await api('/api)/admin/logout', { method: => 'POST' });
    location.reload();
  });
  ...
  async function showPanel() {
    ...
    if (!me.authenticated) return location.reload();
    sessionStorage.setItem('xyven_csrf', me.csrf);
    loadOverview();
  }
  ...
  async function loadOverview() {
    ...
    const cards = [
      ['Total Users', s.total_users, false],
      ['Active Users', s.active_users, false],
      ['Suspended Users', s.suspended_users, false],
      ['Total Referrals', s.total_referrals, false],
      ['Valid Referrals', s.valid_referrals, false],
      ['Total Rewards', s.total_rewards, true],
      ['Total Deposits', s.total_deposits, false],
      ['Pending Deposits', s.pending_deposits, false],
      ['Deposit Value', s.deposit_value, true],
      ['Total Withdrawals', s.total_withdrawals, false],
      ['Pending Withdrawals', s.pending_withdrawals, false],
      ['Completed Withdrawals', s.completed_withdrawals, false],
      ['Rejected Withdrawals', s.rejected_withdrawals, false],
      ['Withdrawn Value', s.withdrawn_value, true],
    ];
    document.getElementById('admin-stats').innerHTML = cards.map(([label, v, money]) => `
  ...
  async function loadUsers(q = '') {
    ...
    if (!d.users.length) { wrap.innerHTML = emptyState('No users found.', 'users'); return; }
  ...
  async function viewUser(id) {
        ...
        <div class="glass modal">
          ...
          <div class="table-wrap mt-14">
            <table><thead><tr><th>Txn</th><th>Type</th><th>Amount</th><th>Status</th></tr></thead><tbody>
              ${d.transactions.slice(0, 8).map((                <tr>
                  <td class="mono-cell">${esc(t.txn_id)}</td>
                  <td>${esc(t.type)}</td>
                  <td>${t.direction === 'credit' ? '+' : '-'}${fmtMoney(t.amount_paise)}</td>
                  <td><span class="${statusCls(t.status)}">${esc(t.status)}</span></td>
                </tr>`).join('')}
            </tbody></table>
          </div>
          <h4 style="margin-top:16px">Adjust Balance</h4>
          <div class="field mt-14">
            <input class="input" id="adj-amount" type="number" placeholder="e.g. 50 or -20" step="1">
          </div>
          <div class="field">
            <input class="input" id="adj-reason" placeholder="Reason">
          </div>
    ...
    document.getElementById('modal-bg').onclick = (e) => { if (e.target.id === 'modal-bg') document.getElementById('modal-root').innerHTML = ''; };
  ...
  async function suspendUser(id) {
    const ok = await promptConfirm('Suspend account?', 'The user will be logged out and blocked. This is logged.', 'Suspend', true);
    if (!ok) return;
  ...
  async function loadWithdrawals() {
    ...
    if (!d.withdrawals.length) { wrap.innerHTML = emptyState('No withdrawal requests.', 'banknote'); return; }
    wrap.innerHTML = `<table><thead><tr><th>Request ID</th><th>User</th><th>Amount</th><th>UPI ID</th><th>Status</th><th>Date</th><th>Actions</th></tr></thead><tbody>` +
      d.withdrawals.map((w) => `
        <tr>
          ...
          <td>
            ${w.status === 'pending' ? `
              <button class="btn btn-sm btn-success" data-act="approve" data-rid="${esc(w.request_id)}">Approve</button>
              <button class="btn btn-sm btn-danger" data-act="reject" data-rid="${esc(w.request_id)}">Reject</button>
            ` : ''}
            ${(w.status === 'pending' || w.status === 'approved') ? `
              <button class="btn btn-sm" data-act="complete" data-rid="${esc(w.request_id)}">Complete</button>
            ` : ''}
          </td>
        </tr>`).join('') + `</tbody></table>`;
    ...
    wrap.querySelectorAll('[data-act]').forEach((b) => b.onclick = async () => {
      ...
      if (!ok) return;
      let note = '';
      if (act === 'reject') note = prompt('Optional reason for rejection:', '') || '';
      try {
        await api(`/api/admin/withdrawals/${rid}/${act}`, { method: 'POST', body: JSON.stringify({ note }) });
        toast(`Withdrawal ${act}d.`, 'success');
        loadWithdrawals();
      } catch (e) { toast(e.message, 'error'); }
    });
  }
  ...
  async function loadDeposits() {
    ...
    if (!d.deposits.length) { wrap.innerHTML = emptyState('No deposit requests.', 'receipt'); return; }
    wrap.innerHTML = `<table><thead><tr><th>Request ID</th><th>User</th><th>Amount</th><th>Reference</th><th>Status</th><th>Date</th><th>Actions</th></tr></thead><tbody>` +
      d.deposits.map((r) => `
        <tr>
          ...
          <td>
            ${r.status === 'pending' ? `
              <button class="btn btn-sm btn-success" data-act="verify" data-rid="${esc(r.request_id)}">Verify</button>
              <button class="btn btn-sm btn-danger" data-act="reject" data-rid="${esc(r.request_id)}">Reject</button>
            ` : ''}
          </td>
        </tr>`).join('') + `</tbody></table>`;
    ...
    wrap.querySelectorAll('[data-act]').forEach((b) => b.onclick = async () => {
      const act = b.dataset.act, rid = b.dataset.rid;
      const ok = await promptConfirm(`Confirm ${act} deposit`,
        `Request ${rid}? Verify only after confirming payment receipt with the payment provider.`,
        act === 'verify' ? 'Verify' : 'Reject', act === 'reject');
      if (!ok) return;
      let note = '';
      if (act === 'reject') note = prompt('Optional rejection reason:', '') || '';
      try {
        await api(`/api/admin/deposits/${rid}/${act}`, { method: 'POST', body: JSON.stringify({ note }) });
        toast(`Deposit ${act}ed.`, 'success');
        loadDeposits();
      } catch (e) { toast(e.message, 'error'); }
    });
  }
  ...
  async function loadCoupons() {
    ...
    if (!d.coupons.length) { wrap.innerHTML = emptyState('No coupons yet.', 'ticket'); return; }
  ...
  document.getElementById('new-coupon')?.addEventListener('click', () => {
    document.getElementById('modal-root').innerHTML = `
      <div class="modal-bg" id="modal-bg">
        <div class="glass modal">
          <h3>New Coupon</h3>
  ...
  async function loadTxns() {
    ...
    if (!d.transactions.length) { wrap.innerHTML = emptyState('No transactions.', 'list'); return; }
    wrap.innerHTML = `<table><thead><tr><th>Txn ID</th><th>User</th><th>Type</th><th>Direction</th><th>Amount</th><th>Status</th><th>Date</th></tr></thead><tbody>` +
      d.transactions.map((t) => `
        <tr>
          ...
          <td>${t.direction === 'credit' ? '+' : '-'}${fmtMoney(t.amount_paise)}</td>
          <td><span class="${statusCls(t.status)}">${esc(t.status)}</span></td>
          <td class="mono-cell">${fmtDateTime(t.created_at)}</td>
        </tr>`).join('') + `</tbody></table>`;
  }
  ...
  async function loadAudit() {
    ...
    if (!d.logs.length) { wrap.innerHTML = emptyState('No audit entries.', 'scroll-text'); return; }
    wrap.innerHTML = `<table><thead><tr><th>When</th><th>Admin</th><th>Action</th><th>Target</th></tr></thead><tbody>` +
      d.logs.map((l) => `
        <tr>
          <td class="mono-cell">${fmtDateTime(l.created_at)}</td>
          <td>${esc(l.admin_username || '—')}</td>
          <td>${esc(l.action)}</td>
          <td class="small">${esc(l.target_type || '')} ${l.target_id ? '#' + esc(l.target_id) : ''}</td>
        </tr>`).join('') + `</tbody></table>`;
  }
  ...
  document.querySelector('[name="audience"]')?.addEventListener('change', (e) => {
    document.getElementById('single-user-wrap').style.display = e.target.value === 'single' ? 'block' : 'none';
  });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  initLang();
  applyI18n();
  // Bootstrap CSRF for auth page only
