...
function startCanvas() {
  ...
  function resize() {
    ...
    for (let i = 0; i < count; i++) {
      dots.push({ x: Math.random() * w, y: Math.random() * h, vx: (Math.random() - .5) * .25, vy: (Math.random() - .5) * .25, r: Math.random() * 1.3 + .3 });
    }
  }
  resize();
...
function initAuthPage() {
  ...
  signupForm.addEventListener('submit', async function(e) {
    ...
    try {
      ...
      setTimeout(function() { location.href = '/app'; }, 600);
    } catch (err) {
      toast(err.message || 'Signup failed.', 'error', 7000);
      if (btn) btn.disabled = false;
    }
  });
...
function initAppPage() {
  ...
  const state = { user: null, referrals: null, unread: 0 };
  ...
  async function loadRefer() {
    ...
    if (!wrap) return;
    if (!d.history.length) wrap.innerHTML = emptyState('No referrals yet.', 'users');
    else {
      wrap.innerHTML = '<table><thead><tr><th>User</th><th>Status</th><th>Joined</th></tr></thead><tbody>' +
        d.history.map(function(r) {
  ...
  if (cc) cc.addEventListener('click', function() {
    ...
    navigator.clipboard.writeText(code).then(function() { toast('Referral code copied.', 'success'); });
  });
  const cl = document.getElementById('copy-link');
  if (cl) cl.addEventListener('click', function() {
    const link = document.getElementById('ref-link').value;
    navigator.clipboard.writeText(link).then(function() { toast('Referral link copied.', 'success'); });
  });
    ...
    if (!d.deposits.length) wrap.innerHTML = emptyState('No deposit requests yet.', 'receipt');
      ...
      wrap.innerHTML = '<table><thead><tr><th>Request ID</th><th>Amount</th><th>Status</th><th>Date</th></tr></thead><tbody>' +
        d.deposits.map(function(r) {
          return '<tr><td class="mono-cell">' + esc(r.request_id) + '</td><td>' + fmtMoney(r.amount_paise) + '</td><td><span class="' + statusCls(r.status) + '">' + esc(r.status) + '</span></td><td class="mono-cell">' + fmtDate(r.created_at) + '</td></tr>';
        }).join('') + '</tbody></table>';
    }
    applyLucide();
  }
  ...
  if (depForm) depForm.addEventListener('submit', async function(e) {
    e.preventDefault();
...
function initAdminPage() {
  ...
  async function loadUsers(q) {
    ...
    wrap.querySelectorAll('[data-view-user]').forEach(function(b) {
      b.onclick = function() { viewUser(Number(b.dataset.viewUser)); };
    });
    wrap.querySelectorAll('[data-suspend]').forEach(function(b) {
      b.onclick = function() { suspendUser(Number(b.dataset.suspend)); };
    });
    wrap.querySelectorAll('[data-unsuspend]').forEach(function(b) {
      b.onclick = function() { unsuspendUser(Number(b.dataset.unsuspend)); };
    });
  }
  ...
  async function viewUser(id) {
    ...
    const root = document.getElementById('modal-root');
    if (!root) return;
    root.innerHTML = '<div class="modal-bg" id="modal-bg"><div class="glass modal">' +
      ...
      '<div class="table-wrap mt-14"><table><thead><tr><th>Txn</th><th>Type</th><th>Amount</th><th>Status</th></tr></thead><tbody>' +
        d.transactions.slice(0, 8).map(function(t) {
          return '<tr><td class="mono-cell">' + esc(t.txn_id) + '</td><td>' + esc(t.type) + '</td><td>' + (t.direction === 'credit' ? '+' : '-') + fmtMoney(t.amount_paise) + '</td><td><span class="' + statusCls(t.status) + '">' + esc(t.status) + '</span></td></tr>';
        }).join('') +
      '</tbody></table></div>' +
    ...
    wrap.innerHTML = '<table><thead><tr><th>Code</th><th>Reward</th><th>Uses</th><th>Per User</th><th>Expires</th><th>Status</th><th>Actions</th></tr></thead><tbody>' +
      d.coupons.map(function(c) {
        return '<tr>' +
          ...
          '<td><button class="btn btn-sm btn-ghost" data-toggle="' + c.id + '" data-active="' + c.active + '">' + (c.active ? 'Disable' : 'Enable') + '</button> <button class="btn btn-sm btn-danger" data-del="' + c.id + '">Delete</button></td>' +
        '</tr>';
      }).join('') + '</tbody></table>';
  ...
  if (nc) nc.addEventListener('click', function() {
    const root = document.getElementById('modal-root');
    if (!root) return;
    root.innerHTML = '<div class="modal-bg" id="modal-bg"><div class="glass modal"><h3>New Coupon</h3>' +
      ...
      '<div class="modal-actions"><button class="btn btn-ghost btn-sm" id="modal-cancel">Cancel</button><button class="btn btn-sm" id="c-save">Create</button></div>' +
    '</div></div>';
    applyLucide();
    document.getElementById('modal-cancel').onclick = function() { root.innerHTML = ''; };
    document.getElementById('modal-bg').onclick = function(e) { if (e.target.id === 'modal-bg') root.innerHTML = ''; };
  ...
  async function loadTxns() {
    ...
    wrap.innerHTML = '<table><thead><tr><th>Txn ID</th><th>User</th><th>Type</th><th>Direction</th><th>Amount</th><th>Status</th><th>Date</th></tr></thead><tbody>' +
      d.transactions.map(function(t) {
        return '<tr><td class="mono-cell">' + esc(t.txn_id) + '</td><td>@' + esc(t.username) + '</td><td>' + esc(t.type) + '</td><td>' + esc(t.direction) + '</td><td>' + (t.direction === 'credit' ? '+' : '-') + fmtMoney(t.amount_paise) + '</td><td><span class="' + statusCls(t.status) + '">' + esc(t.status) + '</span></td><td class="mono-cell">' + fmtDateTime(t.created_at) + '</td></tr>';
      }).join('') + '</tbody></table>';
  }
  ...
  async function loadAudit() {
    ...
    wrap.innerHTML = '<table><thead><tr><th>When</th><th>Admin</th><th>Action</th><th>Target</th></tr></thead><tbody>' +
      d.logs.map(function(l) {
        return '<tr><td class="mono-cell">' + fmtDateTime(l.created_at) + '</td><td>' + esc(l.admin_username || '—') + '</td><td>' + esc(l.action) + '</td><td class="small">' + esc(l.target_type || '') + ' ' + (l.target_id ? '#' + esc(l.target_id) : '') + '</td></tr>';
      }).join('') + '</tbody></table>';
  }
