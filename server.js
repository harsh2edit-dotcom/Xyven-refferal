...
const UPI_NAME = process.env.UPI_NAME || 'XYVEN Referral';
...
let connString = (process.env.DATABASE_URL || '').replace(/[?&]channel_binding=require/, '');
...
/* ---------- UTILS ---------- */
const hashIp = (ip) => ip ? crypto.createHmac('sha256', IP_SECRET).update(String(ip)).digest('hex') : null;
const clientIp = (req) => {
...
const validUtr = (u) => /^[A-Za-z0-9]{8,32}$/.test(u);
...
app.use('/api', (req, res, next) => {
  ...
  if (!sent || sent !== req.session.csrf) {
    return res.status(403).json({ error: 'CSRF', message: 'Security token invalid. Please reload.' });
  }
  next();
});
...
const signupLimiter = lim(10, 60 * 60 * 1000, 'Too many signup attempts.');
const loginLimiter = lim(30, 15 * 60 * 1000, 'Too many login attempts.');
const actionLimiter = lim(60, 60 * 1000, 'Too many requests.');
...
app.post('/api/signup', signupLimiter, async (req, res) => {
  try {
    ...
    const result = await tx(async (c) => {
      // ===== IP LOCK: one account per IP =====
      const ipCheck = await c.query('SELECT id, username FROM users WHERE signup_ip_hash=$1 LIMIT 1', [ipHash]);
      if (ipCheck.rows.length) {
        const err = new Error('Your IP is already used. Don\'t try to be scam.');
        err.status = 403;
        err.code = 'IP_ALREADY_USED';
        throw err;
      }
      ...
      if (dup.rows.length) {
        ...
        const err = new Error(clash.username.toLowerCase() === username.toLowerCase() ? 'That username is already taken.' : 'That email is already registered.');
        err.status = 409;
        throw err;
      }
      ...
      for (let i = 0; i < 10; i++) {
        ...
        try {
          ...
          if (e.code === '23505' && (e.constraint || '').includes('signup_ip_hash')) {
            const err = new Error('Your IP is already used. Don\'t try to be scam.');
            err.status = 403; err.code = 'IP_ALREADY_USED';
            throw err;
          }
          throw e;
        }
      ...
      try {
        await notify(c, user.id, 'Welcome to XYVEN Referral',
          'Your account is ready. Share your referral code to start earning.', 'account');
      } catch (_) {}
        ...
        try {
          const refIns = await c.query(
            `INSERT INTO referrals (referrer_id,referred_id,status,reason,signup_ip_hash,device_hash)
             VALUES ($1,$2,'valid',$3,$4,$5) RETURNING id`,
            [referrer.id, user.id, null, ipHash, deviceHash]);
          ...
          await notify(c, referrer.id, 'Referral reward credited', 'You earned ₹' + (R.REFERRER / 100) + ' because @' + username + ' joined with your code.', 'reward');
          await notify(c, user.id, 'Signup bonus received', '₹' + (R.REFERRED / 100) + ' signup bonus has been credited.', 'reward');
    ...
    res.status(e.status || 500).json({ error: e.code || 'SERVER', message: e.status ? e.message : 'Signup failed: ' + e.message });
  }
});
...
app.post('/api/login', loginLimiter, async (req, res) => {
  try {
    ...
    const { rows } = await pool.query('SELECT * FROM users WHERE LOWER(username)=LOWER($1) OR LOWER(email)=LOWER($1) LIMIT 1', [username]);
    const u = rows[0];
    ...
    if (u.status === 'suspended') return res.status(403).json({ error: 'SUSPENDED', message: 'Account suspended.' });
...
app.get('/api/deposits', requireAuth, async (req, res) => {
  try {
    const { rows } = await pool.query(
      `SELECT request_id, amount_paise, method, upi_id, utr_number, reference_id, status, admin_note, created_at, processed_at,
              (screenshot_data IS NOT NULL) AS has_screenshot
       FROM deposits WHERE user_id=$1 ORDER BY created_at DESC LIMIT 50`, [req.user.id]);
    res.json({ required: R.REQUIRED_DEPOSIT, verified: req.user.deposit_verified, upiId: UPI_ID, upiName: UPI_NAME, deposits: rows });
  } catch (e) { res.status(500).json({ error: 'SERVER', message: e.message }); }
});
...
app.post('/api/deposits', requireAuth, actionLimiter, async (req, res) => {
  try {
    ...
    const utrNumber = clean(req.body.utrNumber || '', 40).toUpperCase();
    const screenshotData = String(req.body.screenshot || '');
    ...
    if (amountPaise < 100) return res.status(400).json({ error: 'VALIDATION', message: 'Minimum deposit amount is ₹1.' });
    if (!utrNumber || !validUtr(utrNumber)) return res.status(400).json({ error: 'VALIDATION', message: 'Enter a valid UTR / Reference number (8-32 chars).' });
    if (!screenshotData || screenshotData.indexOf('data:image/') !== 0) return res.status(400).json({ error: 'VALIDATION', message: 'Please attach a payment screenshot.' });
    if (screenshotData.length > 3 * 1024 * 1024) return res.status(400).json({ error: 'VALIDATION', message: 'Screenshot too large. Please use a smaller image.' });
    ...
    const out = await tx(async (c) => {
      ...
      const rid = genId('DEP');
      const ins = await c.query(
        `INSERT INTO deposits (request_id,user_id,amount_paise,method,upi_id,utr_number,reference_id,screenshot_data,note)
         VALUES ($1,$2,$3,'upi',$4,$5,$5,$6,$7)
         RETURNING request_id, amount_paise, method, upi_id, utr_number, status, created_at`,
        [rid, req.user.id, amountPaise, UPI_ID, utrNumber, screenshotData, note]);
...
app.post('/api/withdrawals', requireAuth, actionLimiter, async (req, res) => {
  try {
    ...
    if (!validUpi(upiId)) return res.status(400).json({ error: 'VALIDATION', message: 'Enter a valid UPI ID.' });
    ...
    const result = await tx(async (c) => {
      ...
      await notify(c, u.id, 'Withdrawal submitted', 'Your withdrawal request ' + rid + ' for ₹' + (amountPaise / 100).toFixed(2) + ' is pending review.', 'withdrawal');
      return ins.rows[0];
    });
...
app.post('/api/coupons/redeem', requireAuth, actionLimiter, async (req, res) => {
  ...
  try {
    const out = await tx(async (c) => {
      ...
      await notify(c, req.user.id, 'Coupon reward credited', '₹' + (coupon.reward_paise / 100).toFixed(2) + ' credited from coupon ' + coupon.code + '.', 'reward');
...
app.get('/api/admin/recent', async (req, res) => {
  try {
    const [users, deposits, withdrawals] = await Promise.all([
      pool.query(`SELECT id, name, username, email, referral_code, balance_paise, status, created_at FROM users ORDER BY created_at DESC LIMIT 10`),
      pool.query(`SELECT request_id, amount_paise, utr_number, status, created_at, u.username FROM deposits d JOIN users u ON u.id=d.user_id ORDER BY d.created_at DESC LIMIT 10`),
      pool.query(`SELECT request_id, amount_paise, upi_id, status, created_at, u.username FROM withdrawals w JOIN users u ON u.id=w.user_id ORDER BY w.created_at DESC LIMIT 10`),
    ]);
    res.json({ users: users.rows, deposits: deposits.rows, withdrawals: withdrawals.rows });
  } catch (e) { res.status(500).json({ error: 'SERVER', message: e.message }); }
});
...
app.get('/api/admin/users/:id', async (req, res) => {
  try {
    ...
    const [refs, txns, deps, wds, notifs] = await Promise.all([
      ...
      pool.query(`SELECT request_id,amount_paise,utr_number,status,created_at FROM deposits WHERE user_id=$1 ORDER BY created_at DESC LIMIT 30`, [id]),
...
app.post('/api/admin/withdrawals/:rid/:action', async (req, res) => {
  ...
  if (['approve', 'reject', 'complete'].indexOf(action) === -1) return res.status(400).json({ error: 'BAD_ACTION' });
  const note = clean(req.body.note || '', 300);
  try {
    await tx(async (c) => {
      ...
      if (action === 'approve') {
        ...
        if (['pending', 'approved'].indexOf(w.status) === -1) { const e = new Error('Already finalised.'); e.status = 400; throw e; }
        ...
        if (['rejected', 'completed'].indexOf(w.status) !== -1) { const e = new Error('Already finalised.'); e.status = 400; throw e; }
...
app.get('/api/admin/deposits', async (req, res) => {
  try {
    const { rows } = await pool.query(`SELECT d.id, d.request_id, d.amount_paise, d.method, d.upi_id, d.utr_number, d.reference_id, d.note, d.status, d.admin_note, d.created_at, d.processed_at, u.username, u.id AS user_id, (d.screenshot_data IS NOT NULL) AS has_screenshot FROM deposits d JOIN users u ON u.id=d.user_id ORDER BY d.created_at DESC LIMIT 200`);
    res.json({ deposits: rows });
  } catch (e) { res.status(500).json({ error: 'SERVER', message: e.message }); }
});

app.get('/api/admin/deposits/:rid/screenshot', async (req, res) => {
  try {
    const { rows } = await pool.query('SELECT screenshot_data FROM deposits WHERE request_id=$1', [req.params.rid]);
    if (!rows[0] || !rows[0].screenshot_data) return res.status(404).json({ error: 'NOT_FOUND' });
    res.json({ screenshot: rows[0].screenshot_data });
  } catch (e) { res.status(500).json({ error: 'SERVER', message: e.message }); }
});

app.post('/api/admin/deposits/:rid/:action', async (req, res) => {
  const { rid, action } = req.params;
  if (['verify', 'reject'].indexOf(action) === -1) return res.status(400).json({ error: 'BAD_ACTION' });
  const note = clean(req.body.note || '', 300);
  try {
    await tx(async (c) => {
      ...
      if (action === 'verify') {
        ...
        await c.query(
          ...
          [txn, d.user_id, amt, 'Deposit verified (UTR ' + d.utr_number + ')', JSON.stringify({ requestId: rid })]);
  ...
  try {
    ...
    for (const k in map) {
      if (req.body[k] !== undefined) {
        ...
        params.push(v); fields.push(map[k] + ' = $' + params.length);
      }
    }
...
app.post('/api/admin/notifications/broadcast', async (req, res) => {
  try {
    ...
    if (audience === 'all') {
      ...
      count = r.rowCount;
    } else if (userId) { await notify(null, userId, title, body, 'announcement'); count = 1; }
...
(async () => {
  ...
  try {
    ...
    console.log('[db] OK. Users: ' + test.rows[0].c);
  } catch (e) {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log('====================================');
    console.log('XYVEN live on port ' + PORT);
