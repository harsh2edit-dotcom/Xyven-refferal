...
/* ---- CSRF token (issued on every request if missing) ---- */
  ...
  try {
    ...
    if (!rows[0]) {
      return req.session.destroy(() => {
        res.clearCookie('xyven.sid');
        next();
      });
    }
    if (rows[0].status === 'suspended') {
      return res.status(403).json({ error: 'SUSPENDED', message: 'Account suspended. Contact @xyvensupport.' });
    }
    req.user = rows[0];
    next();
  } catch (e) { next(e); }
...
app.post('/api/signup', signupLimiter, async (req, res) => {
  try {
    ...
    // Regenerate session for the new user, then issue a FRESH csrf token
    req.session.regenerate((err) => {
      if (err) return res.status(500).json({ error: 'SESSION', message: 'Session error.' });
      req.session.userId = result.id;
      req.session.csrf = genCsrf(); // IMPORTANT: new token for new session
      res.status(201).json({
        ok: true,
...
app.post('/api/login', loginLimiter, async (req, res) => {
  try {
    ...
    req.session.regenerate((err) => {
      ...
      req.session.csrf = genCsrf(); // IMPORTANT: new token
...
/* ---- Session info (NO AUTH REQUIRED — this is the CSRF bootstrap) ---- */
app.get('/api/me', (req, res) => {
  res.json({
    ...
    user: req.user ? {
      ...
      balance: Number(req.user.balance_paise),
...
/* Admin session check (NO AUTH REQUIRED — CSRF bootstrap) */
